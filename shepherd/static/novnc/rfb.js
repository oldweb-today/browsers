define(['exports', './util/logging.js', './util/strings.js', './util/browser.js', './util/eventtarget.js', './display.js', './input/keyboard.js', './input/mouse.js', './util/cursor.js', './websock.js', './des.js', './input/keysym.js', './input/xtscancodes.js', './inflator.js', './encodings.js', './util/polyfill.js'], function (exports, _logging, _strings, _browser, _eventtarget, _display, _keyboard, _mouse, _cursor, _websock, _des, _keysym, _xtscancodes, _inflator, _encodings) {
    'use strict';
    ...
    var RFB = function (_EventTargetMixin) {
        ...
        function RFB(target, url, options) {
            ...
            for (var i = 0; i < 4; i++) {
                _this._FBU.zlibs[i] = new _inflator2.default();
            }

            _this._destBuff = null;
            _this._paletteBuff = new Uint8Array(1024); // 256 * 4 (max palette size * max bytes-per-pixel)

            _this._rre_chunk_sz = 100;
            ...
            _this._encHandlers[_encodings.encodings.encodingRaw] = RFB.encodingHandlers.RAW.bind(_this);
            _this._encHandlers[_encodings.encodings.encodingCopyRect] = RFB.encodingHandlers.COPYRECT.bind(_this);
            _this._encHandlers[_encodings.encodings.encodingRRE] = RFB.encodingHandlers.RRE.bind(_this);
            _this._encHandlers[_encodings.encodings.encodingHextile] = RFB.encodingHandlers.HEXTILE.bind(_this);
            _this._encHandlers[_encodings.encodings.encodingTight] = RFB.encodingHandlers.TIGHT.bind(_this, false);
            _this._encHandlers[_encodings.encodings.encodingTightPNG] = RFB.encodingHandlers.TIGHT.bind(_this, true);

            _this._encHandlers[_encodings.encodings.pseudoEncodingDesktopSize] = RFB.encodingHandlers.DesktopSize.bind(_this);
            _this._encHandlers[_encodings.encodings.pseudoEncodingLastRect] = RFB.encodingHandlers.last_rect.bind(_this);
            _this._encHandlers[_encodings.encodings.pseudoEncodingCursor] = RFB.encodingHandlers.Cursor.bind(_this);
            _this._encHandlers[_encodings.encodings.pseudoEncodingQEMUExtendedKeyEvent] = RFB.encodingHandlers.QEMUExtendedKeyEvent.bind(_this);
            _this._encHandlers[_encodings.encodings.pseudoEncodingExtendedDesktopSize] = RFB.encodingHandlers.ExtendedDesktopSize.bind(_this);
            ...
            _this._sock.on('message', _this._handle_message.bind(_this));
        ...
        _createClass(RFB, [{
            ...
            value: function _print_stats() {
                ...
                Object.keys(stats).forEach(function (key) {
                    return Log.Info("    " + (0, _encodings.encodingName)(key) + ": " + stats[key][1] + " rects");
                });
            }
        }, {
            key: '_focusCanvas',
            ...
            value: function _handle_security_failure(context, security_result_status) {
                ...
                if (reason !== "") {
                    this.dispatchEvent(new CustomEvent("securityfailure", { detail: { status: security_result_status, reason: reason } }));

                    return this._fail("Security negotiation failed" + context + " (reason: " + reason + ")");
                } else {
                    this.dispatchEvent(new CustomEvent("securityfailure", { detail: { status: security_result_status } }));
    ...
    RFB.encodingHandlers = {
        ...
        TIGHT: function TIGHT(isTightPNG) {
            ...
            var resetStreams = 0;
            var streamId = -1;
            var decompress = function decompress(data, expected) {
                for (var i = 0; i < 4; i++) {
                    if (resetStreams >> i & 1) {
                        _this4._FBU.zlibs[i].reset();
                        Log.Info("Reset zlib stream " + i);
                    }
                }

                //const uncompressed = this._FBU.zlibs[streamId].uncompress(data, 0);
                var uncompressed = _this4._FBU.zlibs[streamId].inflate(data, true, expected);
            ...
            var indexedToRGBX2Color = function indexedToRGBX2Color(data, palette, width, height) {
                // Convert indexed (palette based) image data to RGB
                // TODO: reduce number of calculations inside loop
                var dest = _this4._destBuff;
            ...
            var indexedToRGBX = function indexedToRGBX(data, palette, width, height) {
                // Convert indexed (palette based) image data to RGB
                var dest = _this4._destBuff;
            ...
            var handlePalette = function handlePalette() {
                var numColors = rQ[rQi + 2] + 1;
                var paletteSize = numColors * 3;
                _this4._FBU.bytes += paletteSize;
                if (_this4._sock.rQwait("TIGHT palette " + cmode, _this4._FBU.bytes)) {
                    return false;
                }

                var bpp = numColors <= 2 ? 1 : 8;
                var rowSize = Math.floor((_this4._FBU.width * bpp + 7) / 8);
                var raw = false;
                if (rowSize * _this4._FBU.height < 12) {
                    raw = true;
                    cl_header = 0;
                    cl_data = rowSize * _this4._FBU.height;
                ...
                _this4._FBU.bytes += cl_header + cl_data;
                if (_this4._sock.rQwait("TIGHT " + cmode, _this4._FBU.bytes)) {
                    return false;
                }

                // Shift ctl, filter id, num colors, palette entries, and clength off
                _this4._sock.rQskipBytes(3);
                //const palette = this._sock.rQshiftBytes(paletteSize);
                _this4._sock.rQshiftTo(_this4._paletteBuff, paletteSize);
                _this4._sock.rQskipBytes(cl_header);

                if (raw) {
                    data = _this4._sock.rQshiftBytes(cl_data);
                } else {
                    data = decompress(_this4._sock.rQshiftBytes(cl_data), rowSize * _this4._FBU.height);
                }

                // Convert indexed (palette based) image data to RGB
                var rgbx = void 0;
                if (numColors == 2) {
                    rgbx = indexedToRGBX2Color(data, _this4._paletteBuff, _this4._FBU.width, _this4._FBU.height);
                } else {
                    rgbx = indexedToRGBX(data, _this4._paletteBuff, _this4._FBU.width, _this4._FBU.height);
                }

                _this4._display.blitRgbxImage(_this4._FBU.x, _this4._FBU.y, _this4._FBU.width, _this4._FBU.height, rgbx, 0, false);
            ...
            var handleCopy = function handleCopy() {
                var raw = false;
                var uncompressedSize = _this4._FBU.width * _this4._FBU.height * 3;
                ...
                _this4._FBU.bytes = 1 + cl_header + cl_data;
                if (_this4._sock.rQwait("TIGHT " + cmode, _this4._FBU.bytes)) {
                    return false;
                }

                // Shift ctl, clength off
                _this4._sock.rQshiftBytes(1 + cl_header);

                if (raw) {
                    data = _this4._sock.rQshiftBytes(cl_data);
                } else {
                    data = decompress(_this4._sock.rQshiftBytes(cl_data), uncompressedSize);
                }

                _this4._display.blitRgbImage(_this4._FBU.x, _this4._FBU.y, _this4._FBU.width, _this4._FBU.height, data, 0, false);
        ...
        Cursor: function Cursor() {
            ...
            this._cursor.change(this._sock.rQshiftBytes(pixelslength), this._sock.rQshiftBytes(masklength), x, y, w, h);
