var CBrowser = function(reqid, target_div, init_params) {
    ...
    function can_support_mediasource() {
    ...
    function setup_browser() {
        ...
        // check Audio supported by the current browser
        if (can_support_mediasource()) {
            add_audio_to_request(req_params, "opus");
        }
        init_browser();
    }

    function add_audio_to_request(request, audio_type) {
        if (request.hasOwnProperty("audio")) {
            request["audio"] += "|" + audio_type;
        } else {
            request["audio"] = audio_type;
        }
    }
