...
#=============================================================================
from server.helper.audio import determine_audio_type, get_audio_env
...
class DockerController(object):
    ...
    def init_new_browser(self, reqid, host, width=None, height=None, requested_audio="opus"):
        req_key = 'req:' + reqid
        ...
        if requested_audio is not None:
            audio = determine_audio_type(requested_audio, self.get_browser_info(browser))
        else:
            audio = None

        env = get_audio_env(audio)

        env['URL'] = url
        env['TS'] = ts
        env['BROWSER'] = browser
        env["AUDIO_TYPE"] = audio
